...
from app.models import Chunk, ChunkingStrategy, LoadedDocument
    ...
    *,
    ...
    def split_one(text: str) -> list[tuple[str, str | None]]:
        ...
        if strategy == ChunkingStrategy.STRUCTURE_AWARE:
            return _structure_aware_split(text, structure_max_section_size)
...
# --------------------------------------------------------------------------
def _structure_aware_split(text: str, max_section_size: int) -> list[tuple[str, str | None]]:
    if not re.search(r"(?m)^#{1,6}\s", text):
        # No headings in this text (e.g. a plain PDF page) -> fall back to
        # a plain recursive split with no heading metadata, rather than
        # returning the whole page as one giant untitled chunk.
        return _fixed_size_split(text, max_section_size, max_section_size // 8)

    splitter = MarkdownHeaderTextSplitter(headers_to_split_on=_MD_HEADERS, strip_headers=True)
