...
def test_empty_store_is_never_a_duplicate(small_vector_store):
    result = check_duplicate([1.0, 0.0, 0.0], small_vector_store, threshold=0.95)
    assert result.is_duplicate is False
    assert result.duplicate_of is None


def test_identical_embedding_is_flagged_as_duplicate(small_vector_store):
    small_vector_store.add("a", [1.0, 0.0, 0.0], "original chunk text", {"source_document": "x"})
...
def test_dissimilar_embedding_is_not_a_duplicate(small_vector_store):
    small_vector_store.add("a", [1.0, 0.0, 0.0], "original chunk text", {"source_document": "x"})

    result = check_duplicate([0.0, 1.0, 0.0], small_vector_store, threshold=0.95)

    assert result.is_duplicate is False


def test_threshold_boundary_is_respected(small_vector_store):
    small_vector_store.add("a", [1.0, 0.0, 0.0], "text", {"source_document": "x"})
    # a vector at ~0.90 cosine similarity to [1,0]
    near = [0.90, (1 - 0.90 ** 2) ** 0.5, 0.0]

    strict = check_duplicate(near, small_vector_store, threshold=0.95)
    lenient = check_duplicate(near, small_vector_store, threshold=0.5)
