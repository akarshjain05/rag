...
def make_pipeline(fake_embedder, vector_store, **overrides):
    kwargs = dict(
...
def test_ingest_single_file_populates_both_indexes(fake_embedder, vector_store, md_path):
    pipeline = make_pipeline(fake_embedder, vector_store)
...
def test_reingesting_identical_file_is_fully_deduplicated(fake_embedder, vector_store, md_path):
    pipeline = make_pipeline(fake_embedder, vector_store)
...
def test_ingest_tracks_chunking_strategy_in_metadata(fake_embedder, vector_store, md_path):
    pipeline = make_pipeline(fake_embedder, vector_store)
...
def test_ingest_semantic_strategy_end_to_end(fake_embedder, vector_store, md_path):
    pipeline = make_pipeline(fake_embedder, vector_store, semantic_similarity_threshold=0.5,
...
def test_ingest_files_batch_syncs_sparse_index_once(fake_embedder, vector_store, md_path, txt_path, html_path):
    pipeline = make_pipeline(fake_embedder, vector_store)
...
def test_ingest_files_batch_survives_one_bad_file(fake_embedder, vector_store, md_path, tmp_path):
...
def test_ingest_pdf_preserves_page_numbers_through_pipeline(fake_embedder, vector_store, pdf_path):
    import pytest; pytest.importorskip("pytesseract")
    pipeline = make_pipeline(fake_embedder, vector_store, fixed_chunk_size=1000, fixed_chunk_overlap=0)
