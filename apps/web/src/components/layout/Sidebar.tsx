    ...
    <>
      ...
      <aside className={`fixed md:relative z-50 w-64 h-full bg-surface-sunken border-r border-border-strong flex flex-col transition-transform duration-300 ${mobileMenuOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'}`}>
        <div className="p-6 pb-4 border-b border-border flex items-center justify-between">
          <h1 
            className="font-serif italic font-semibold text-[17px] text-ink tracking-wide cursor-pointer hover:text-accent transition-colors" 
            onClick={() => { setActiveConversationId(null); setCurrentView('chat'); }}
          >
            Vellumiq
          </h1>
          <button 
            className="md:hidden text-ink-secondary hover:text-ink hover:bg-border/50 rounded-md p-1 transition-all"
        ...
        <div className="p-4 border-b border-border">
          <button 
            ...
            className="w-full text-center px-4 py-2 border border-border-strong hover:border-accent hover:text-accent hover:bg-accent-tint rounded-md bg-transparent text-[13px] font-sans text-ink transition-all duration-200"
          >
            + New Research
          </button>
        </div>
        
        <nav className="flex-1 overflow-y-auto p-4 flex flex-col gap-0.5 min-h-0">
          ...
          {(conversations || []).map(c => (
            <div key={c.id} className="group relative flex items-center">
              <button
                ...
                className={`flex-1 text-left font-sans text-[13px] py-2 px-3 rounded-md truncate transition-all duration-150 ${
                  currentView === 'chat' && activeConversationId === c.id 
                    ? 'text-ink bg-surface-card border-l-2 border-accent font-medium' 
                    : 'text-ink-secondary hover:text-ink hover:bg-surface-card/70'
                }`}
              >
                {c.title}
              </button>
              <button
                ...
                className="opacity-0 group-hover:opacity-100 p-1.5 text-ink-muted hover:text-ink hover:bg-border/50 transition-all rounded-md ml-1"
              ...
              {dropdownId === c.id && createPortal(
                <>
                  ...
                  <div 
                    className="fixed z-[101] w-36 bg-surface-card border border-border shadow-lg py-1 rounded-md"
                    style={{ 
                  ...
                  >
                    <button
                      ...
                      className="w-full text-left px-3 py-2 text-xs text-danger hover:bg-danger-tint flex items-center gap-2 transition-colors"
                    >
        ...
        <div className="p-4 border-t border-border flex flex-col gap-1">
          <button
            ...
            className={`text-left text-[13px] font-sans px-3 py-2 rounded-md transition-all duration-150 ${
              currentView === 'knowledge' 
                ? 'text-ink bg-surface-card font-medium' 
                : 'text-ink-secondary hover:text-ink hover:bg-surface-card/70'
            }`}
          >
            Knowledge Base
        ...
        <div className="p-6 pt-4 border-t border-border flex justify-between items-center text-ink-muted">
            <button 
              ...
              className="p-2 hover:text-ink hover:bg-border/50 rounded-md transition-all" 
