...
from app.llm_client import AnthropicLLMClient, OpenAILLMClient, build_llm_client
...
def test_anthropic_client_calls_sdk_correctly(monkeypatch):
    ...
    monkeypatch.setattr("anthropic.Anthropic", lambda api_key: fake_instance)
...
def test_openai_llm_client_calls_sdk_correctly(monkeypatch):
    ...
    monkeypatch.setattr("openai.OpenAI", lambda api_key: fake_instance)
